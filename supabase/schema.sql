...
create table if not exists ballot_status (
  id uuid primary key default gen_random_uuid(),
  project_id uuid not null references projects(id) on delete cascade,
  voter_id uuid not null references profiles(id) on delete cascade,
  is_locked boolean not null default false,
...
create index if not exists idx_project_segments_project_id on project_segments(project_id);
create index if not exists idx_segment_options_segment_id on segment_options(segment_id);
...
create index if not exists idx_ballot_status_project_voter on ballot_status(project_id, voter_id);
...
create policy if not exists "partners read assigned projects" on projects
  for select using (
    exists (
...
create policy if not exists "admins manage projects" on projects
  for all using (
...
create policy if not exists "project segments visible to assigned or admin" on project_segments
  for select using (
...
create policy if not exists "options visible to assigned or admin" on segment_options
  for select using (
    exists (
      select 1
...
create policy if not exists "participants visible to admin or self" on project_participants
  for select using (
    profile_id = auth.uid()
    or exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  );

create policy if not exists "admin manage participants" on project_participants
  for all using (
    exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  ) with check (
    exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
...
create policy if not exists "partners read own votes" on votes
  for select using (
    voter_id = auth.uid()
    or exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  );

create policy if not exists "partners insert own unlocked votes" on votes
...
create policy if not exists "partners update own unlocked votes" on votes
  for update using (
    voter_id = auth.uid()
    and is_locked = false
  ) with check (
...
create policy if not exists "admin manage votes" on votes
  for all using (
    exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  ) with check (
    exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
...
create policy if not exists "partners read own ballot status" on ballot_status
  for select using (
    voter_id = auth.uid()
    or exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  );

create policy if not exists "partners manage own ballot status" on ballot_status
  for all using (
    voter_id = auth.uid()
    or exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  ) with check (
...
create policy if not exists "admin read audit logs" on audit_logs
  for select using (
    exists (select 1 from profiles p where p.id = auth.uid() and p.role = 'admin')
  );

create policy if not exists "system inserts audit logs" on audit_logs
